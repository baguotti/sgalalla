...
| `v0.7.0` | **Socket.io Migration** | 2026-02-06 | Pivoted to WebSockets for reliable production connectivity. <br> ↳ **Meta**: Abandoned WebRTC/UDP due to Fly.io infra limitations <br> ↳ **New**: Socket.io integration on Client & Server <br> ↳ **Fix**: Removed all UDP dependencies |
| `v0.7.1` | **Deployment Fixes** | 2026-02-06 | Fixed Socket.io routing issues. <br> ↳ **Fix**: Removed orphaned port 3000 from fly.toml <br> ↳ **Fix**: Client URL uses standard HTTPS (443) <br> ↳ **Fix**: Added missing socket.io-client dependency |
| `v0.6.18` | **Port 3000 Strategy** | 2026-02-06 | Switched to straight Port 3000 configuration. <br> ↳ **Fix**: Removed `http_service` proxy abstraction <br> ↳ **Fix**: Exposed pure TCP/UDP on Port 3000 <br> ↳ **Fix**: Client now targets `https://...:3000` directly |
| `v0.6.17` | **Production Connectivity** | 2026-02-06 | Final fix using Client-side TURN relay. <br> ↳ **New**: Forced `iceTransportPolicy: 'relay'` on Client <br> ↳ **Fix**: Shifted relay logic from server to client to bypass library limitations |
| `v0.6.14` | **Infra Persistence** | 2026-02-06 | Stabilized WebRTC handshake signaling. <br> ↳ **Fix**: Set `min_machines_running: 1` to prevent mid-handshake restarts <br> ↳ **Fix**: Resolved connection ID expiration (404) |
| `v0.6.13` | **UDP Consolidation** | 2026-02-06 | Unified server logic and network ports. <br> ↳ **Fix**: Consolidated duplicate HTTP servers in `index.ts` <br> ↳ **Fix**: Forced Geckos.io to use UDP port 3000 |
| `v0.6.12` | **Vercel Prod Sync** | 2026-02-06 | Synchronized client code for production. <br> ↳ **Fix**: Pushed `NetworkManager.ts` fixes to `main` branch <br> ↳ **Fix**: Updated client to target port 443 for HTTPS production |
| `v0.6.11` | **Local Network Fix** | 2026-02-06 | Enabled testing via local IP (192.168.x.x). <br> ↳ **Fix**: Updated `isLocal` check and defaulted local port to 3000 |
